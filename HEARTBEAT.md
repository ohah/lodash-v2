...
- 턴 종료 시 보내는 요약이나 알림(디스코드, 오픈클로 등)에는 **반드시** 아래를 포함하세요.
  - **Phase 1 구현 진행률**: `docs/LODASH_IMPLEMENTATION_CHECKLIST.md`에서 `[x]` 개수 / 전체 개수. 예: `Phase 1 구현 47/126 완료`.
  - **이번 턴에 구현한 함수**: 이번 턴에 새로 구현해 `[ ]`→`[x]`로 바꾼 함수가 있으면 함수명(여러 개면 쉼표 구분), 없으면 `없음`.
- 예시 문장: `[lodash-v2 Heartbeat] 완료 — Phase 1 구현 47/126, 이번 턴 구현: differenceBy. 동등성 테스트 84실패, generate-feedback 완료`
