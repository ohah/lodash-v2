...
**edit(search_replace) 실패 시**

- `search_replace`는 `old_string`이 파일 내용과 **공백·줄바꿈까지 정확히** 일치해야 합니다.
- 실패 시: 해당 파일을 `read`로 다시 읽고, 수정할 구역을 **그대로 복사**해 `old_string`에 넣어 재시도.
- 그래도 실패하면: 해당 함수/블록만 새 내용으로 묶어서 `write`로 해당 파일 전체를 덮어쓰는 방식으로 수정하세요.
...
- 턴 종료 시 보내는 요약이나 알림(디스코드, 오픈클로 등)에는 **반드시** 아래를 포함하세요.
  - **Phase 1 구현 진행률**: **항상** `docs/LODASH_IMPLEMENTATION_CHECKLIST.md`를 열어 Phase 1 표에서 `[x]` 개수를 세고, 전체는 **126**으로 계산. 형식: `Phase 1 구현 N/126`. (체크리스트 파일이 없을 때만 "체크리스트 없음"이라고 보고)
  - **이번 턴에 구현한 함수**: 이번 턴에 새로 구현해 `[ ]`→`[x]`로 바꾼 함수가 있으면 함수명(여러 개면 쉼표 구분), 없으면 `없음`.
- 예시 문장: `[lodash-v2 Heartbeat] 완료 — Phase 1 구현 46/126, 이번 턴 구현: 없음. (또는) 이번 턴 구현: dropRightWhile`
...
응답 보내기 전: [ ] 결과 동등성 테스트 [ ] 구현 체크리스트 다음 할 일 한 항목 진행 [ ] 필요 시 커밋·푸시 [ ] 디스코드 알림(구현·푸시 했을 때, WEBHOOK 있으면) [ ] 인수인계 문서 갱신(해당 시) [ ] **알림/요약에 Phase 1 진행률(x/126) + 이번 턴 구현 함수명 포함**

위를 모두 했을 때만 `HEARTBEAT_OK` 또는 이번 턴 요약을 보내세요. 요약·알림에는 **Phase 1 구현 N/126**, **이번 턴 구현: 함수명 또는 없음**을 꼭 넣으세요.
